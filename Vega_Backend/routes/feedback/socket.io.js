...
// const feedbackCtrl = require('../Controller');
// // const messages = require('../../lib/messages');


// module.exports = async (io) => {
//     io.on('connection', async (socket) => {
//         console.log("------------------------Feedback---connection---successfull--------------------------------------");
...
//         socket.on('feedbacklikecomment', async (keys) => {
//             keys.createdby = socket.createdby;
//             const data = await timelineController.feedbacklikecomment(keys);
//             io.emit('feedbacklikecommentcount', data);
//         });
...
//             if (item.action_type == 1 && item.reaction_type > 1) {
//                 message.notification.body = `${item.notificationdesc} reacted on your post`;
//             }
//             else if (item.action_type == 1 && item.reaction_type == 1) {
...
//             if (keys.action_type == 4) {
//                 if (arr && arr[0] && arr[0].assignedtouserid) {

//                     //Sending notification to mobile user
//                     notificationCtrl.sendNotificationToMobileDevices(arr[0].assignedtouserid, message);
...
//                     notificationCtrl.sendNotificationToMobileDevices(arr[1].assignedtouserid, message);
...
//             if (message.notification.title.toLowerCase() == "time") message.notification.title = "Timesheet"

//             console.log("________ASSIGNED USER ID FOR MODULES____________", data[0]);
//             notificationCtrl.sendNotificationToMobileDevices(data[0].assignedtouserid, message);
